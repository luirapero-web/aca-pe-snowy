    ...
    <main className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="flex justify-center gap-2 mt-8 px-4">
        {["TODOS","HOY","NORTE","SUR","CENTRO"].map(f=>(
          <button key={f} onClick={()=>setFiltro(f)} className={`px-4 py-2 rounded-full text-[10px] font-black ${filtro===f?"bg-[#CCFF00] text-black":"bg-white/10"}`}>{f}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 max-w-5xl mx-auto mt-6">
        {filtradas.map(p=>(
          <div key={p.id} className="bg-white text-black rounded-[24px] overflow-hidden">
            ...
            <div className="p-4">
              <h3 className="font-black text-[17px]">{p.nombre}</h3>
              ...
              <button onClick={()=>setAsistire(prev => ({...prev, [p.id]:!prev[p.id] }))} className={`w-full py-3.5 rounded-full text-[12px] font-black mt-3 ${asistire[p.id]? "bg-[#CCFF00] text-black" : "bg-black text-white"}`}>{asistire[p.id]? "✓ ASISTIRÉ" : "ASISTIRÉ"}</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={()=>setModal(null)}>
          <div className="bg-[#111] rounded-[24px] p-5 max-w-sm w-full text-white" onClick={e=>e.stopPropagation()}>
            <h2 className="font-black text-[20px]">{modal.nombre}</h2>
            <div className="bg-black rounded-xl p-3 mt-4 text-[11px] space-y-2">
